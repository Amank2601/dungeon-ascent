...
const markFloor = (p, wid, fid) => {
  const w = { ...(p.worlds[wid] || { completedFloors: [], bossDefeated: false }) };
  if (!w.completedFloors.includes(fid)) w.completedFloors = [...w.completedFloors, fid];
  return { ...p, worlds: { ...p.worlds, [wid]: w }, totalXP: (p.totalXP || 0) + 100 };
};

const markBoss = (p, wid) => {
  const w = { ...(p.worlds[wid] || { completedFloors: [], bossDefeated: false }), bossDefeated: true };
  return { ...p, worlds: { ...p.worlds, [wid]: w }, totalXP: (p.totalXP || 0) + 500 };
};

// ─── TOP BAR ──────────────────────────────────────────────────────────────────
function TopBar({ progress, onMap, user }) {
  const { logout } = useAuth();
  return (
    <div style={ui.topBar}>
      <div style={ui.topLogo} onClick={onMap}>⚔️ DUNGEON ASCENT</div>
      <div style={ui.topCenter}>
        <span style={ui.xp}>✨ {progress.totalXP || 0} XP</span>
      </div>
...
function Game() {
  ...
  useEffect(() => {
    if (!user) return;
    const key = `dungeon_progress_${user.id}`;
    try {
  ...
  return (
    <div style={ui.root}>
      ...
      {scene !== "intro" && (
        <TopBar progress={progress} onMap={() => setScene("worldmap")} user={user} />
      )}
      ...
      <div style={ui.content}>
        ...
        {scene === "battle" && selectedFloor && selectedMonster && (
          <BattleScreen
